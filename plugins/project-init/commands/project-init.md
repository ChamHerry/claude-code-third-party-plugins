...
Skills 是可扩展的专业能力模块，AI 应根据当前环境中实际可用的 skills 进行任务处理。

**使用流程**：

1. **识别任务类型**：分析用户需求，判断是否需要专业技能支持
2. **查看可用技能**：通过系统提供的 `<available_skills>` 列表确认当前可用的技能
3. **匹配最佳技能**：根据技能描述选择最适合当前任务的技能
4. **调用技能**：使用 `Skill` 工具调用对应技能

**在设计阶段使用 Skills**：

- **文档生成类技能**：用于生成格式化的方案文档、报告
- **可视化设计类技能**：用于创建架构图、流程图、设计稿
- **数据分析类技能**：用于性能数据分析、指标可视化

**在实施阶段使用 Skills**：

- **前端开发类技能**：用于创建高质量的 UI 组件和页面
- **文档处理类技能**：用于生成测试报告、用户文档

**Skills 使用原则**：
- **主动匹配**: 发现任务与可用技能匹配时，应主动调用对应技能
- **能力匹配**: 根据技能描述准确匹配任务需求，有匹配就使用
- **质量优先**: 利用技能产出高质量交付物
- **充分利用**: 技能是增强能力的工具，匹配的技能应该被充分使用
...
{根据 SkillsDetection.forTemplate 动态生成}

**设计阶段可用技能**：
{如果 SkillCategories.design 不为空}
- **[技能名称]**: [技能描述] - 用于 [适用场景]
{遍历 design 类技能}

**文档处理可用技能**：
{如果 SkillCategories.document 不为空}
- **[技能名称]**: [技能描述] - 用于 [适用场景]
{遍历 document 类技能}

**数据分析可用技能**：
{如果 SkillCategories.data 不为空}
- **[技能名称]**: [技能描述] - 用于 [适用场景]
{遍历 data 类技能}

**其他可用技能**：
{如果 SkillCategories.other 不为空}
- **[技能名称]**: [技能描述] - 用于 [适用场景]
{遍历 other 类技能}

**使用流程**：

1. **识别任务类型**：分析用户需求，判断是否有匹配的技能
2. **主动匹配技能**：发现任务与上述可用技能匹配时，应主动调用
...
Skills 是可扩展的专业能力模块。当前环境未检测到可用的 Skills。
...
c. 用生成的 Skills 使用规范内容替换模板中的 `### Skills 技能使用规范` 整个章节
